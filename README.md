...
8. ✅ 海外 IP 自动代理 —— GitHub Actions 等海外环境被雨云拒绝连接时，自动抓取国内免费代理绕过拦截（无需手动配置）
...
项目支持两种代理方式：**海外环境自动代理**（免配置）和 **自建代理接口**（可选）。

### 海外 IP 自动代理（免配置）

雨云会对海外数据中心 IP 拒绝连接（`ERR_CONNECTION_REFUSED`），GitHub Actions 的 runner 在海外，直连必然失败。

从 v2.3 起，脚本在检测到 GitHub Actions 环境且未配置 `PROXY_API_URL` 时，会**自动抓取国内免费代理**绕过拦截：

- 使用改进版 [freeproxy](https://github.com/LeapYa/freeproxy) 库，ip2region 本地离线定位 + 找到可用代理即停，秒级完成
- 以 `app.rainyun.com` 为探针并发验证，确保代理能真正连上雨云
- 每个账号每次签到独立获取代理，失败自动重试
- **无需任何配置**，Actions 环境下自动启用
...
- 新增海外 IP 自动代理：GitHub Actions 等海外环境被雨云拒绝连接时，自动抓取国内免费代理绕过拦截（基于改进版 freeproxy，找到可用即停，免配置）
